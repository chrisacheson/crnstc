...
def render_bar(console: Console, current_value: int, maximum_value: int,
    ...
    if bar_width > 0:
        console.draw_rect(x=0, y=45, width=bar_width, height=1, ch=1,
                          bg=color.bar_filled)
...
def render_dungeon_level(console: Console, dungeon_level: int,
    ...
    x, y = location
    console.print(x=x, y=y, string=f"Facility level: {dungeon_level}")


def render_names_at_mouse_location(console: Console, x: int, y: int,
                                   engine: Engine) -> None:
    mouse_x, mouse_y = engine.mouse_location
    names_at_mouse_location = get_names_at_location(x=mouse_x, y=mouse_y,
